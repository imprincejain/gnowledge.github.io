...
This project aims to automate the annual report generation for HBCSE which was previously done manually. The data is collected using a free and open source platform called [KoBoToolbox](https://www.kobotoolbox.org/). The forms on KoBoToolbox are base on [XLSForm](http://xlsform.org/en/). These forms can be simply created using Excel. The XLSForm that are uploaded on KoBoToolbox are then converted to [ODK Xform](https://getodk.github.io/xforms-spec/), which is a popular open standard that allows to author a form with complex functionality like skip logic.
